#include <assert.h>
#include <stdio.h>
#include "SOR_9PT_CROSS_SOR_kernel.hu"
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include<sys/time.h>
...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
	  ...
	  cudaCheckReturn(cudaMalloc((void **) &dev_arr1, (padd >= 4104 ? len1 : len1 + 2) * (4104) * sizeof(int)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_arr2, (padd >= 4104 ? len1 : len1 + 2) * (4104) * sizeof(int)));
	  
	  if (padd <= 4105) {
	    cudaCheckReturn(cudaMemcpy(dev_arr1, arr1, (padd >= 4104 ? len1 : len1 + 2) * (4104) * sizeof(int), cudaMemcpyHostToDevice));
	    cudaCheckReturn(cudaMemcpy(dev_arr2, arr2, (padd >= 4104 ? len1 : len1 + 2) * (4104) * sizeof(int), cudaMemcpyHostToDevice));
	  }
	...
	printf("execution time: %lf\n", t3);
	  ...
	  if (padd <= 4105) {
	    cudaCheckReturn(cudaMemcpy(arr1, dev_arr1, (padd >= 4104 ? len1 : len1 + 2) * (4104) * sizeof(int), cudaMemcpyDeviceToHost));
	    cudaCheckReturn(cudaMemcpy(arr2, dev_arr2, (padd >= 4104 ? len1 : len1 + 2) * (4104) * sizeof(int), cudaMemcpyDeviceToHost));
	  }
