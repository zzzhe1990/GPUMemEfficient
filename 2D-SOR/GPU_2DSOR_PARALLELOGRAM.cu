...
__device__ void _9pt_CROSS_SOR(volatile int* tile1, volatile int* tile2, int newtilePos, int tilePos, int* stride, int* segLengthX){
	tile2[newtilePos] = (tile1[tilePos - 2] + tile1[tilePos - 1] + tile1[tilePos] + tile1[tilePos + 1] + tile1[tilePos + 2] + tile1[tilePos + segLengthX[0]] + tile1[tilePos + segLengthX[0] + segLengthX[0]] + tile1[tilePos - segLengthX[0]] + tile1[tilePos - segLengthX[0] - segLengthX[0]]) / 9;
}

__device__ void _9pt_SQUARE_SOR(volatile int* tile1, volatile int* tile2, int newtilePos, int tilePos, int* stride, int* segLengthX){
...
__device__ void _25pt_SQUARE_SOR(volatile int* tile1, volatile int* tile2, int newtilePos, int tilePos, int* stride, int* segLengthX){
	...
	tile2[newtilePos] = total / 25;
}

__device__ void _13pt_CROSS_SOR(volatile int* tile1, volatile int* tile2, int newtilePos, int tilePos, int* stride, int* segLengthX){
	int total = 0;
	for (int i = -3; i < 0; i++){
		total += tile1[tilePos + i * segLengthX[0]];
	}
	for (int i = 1; i <= 3; i++){
		total += tile1[tilePos + i * segLengthX[0]];
	}
	for (int i = -3; i <= 3; i++){
		total += tile1[tilePos + i];
	}
	tile2[newtilePos] = total / 13;
}

__device__ void _49pt_SQUARE_SOR(volatile int* tile1, volatile int* tile2, int newtilePos, int tilePos, int* stride, int* segLengthX){
	int total = 0;
	for (int i = -3; i <= 3; i++){
		for (int j = -3; j <= 3; j++){
			total += tile1[tilePos + i * segLengthX[0] + j];
	...
	tile2[newtilePos] = total / 49;
}

__device__ void _17pt_CROSS_SOR(volatile int* tile1, volatile int* tile2, int newtilePos, int tilePos, int* stride, int* segLengthX){
	int total = 0;
	for (int i = -4; i < 0; i++){
		total += tile1[tilePos + i * segLengthX[0]];
	}
	...
	tile2[newtilePos] = total / 17;
}

__device__ void _81pt_SQUARE_SOR(volatile int* tile1, volatile int* tile2, int newtilePos, int tilePos, int* stride, int* segLengthX){
	int total = 0;
	for (int i = -4; i <= 4; i++){
		for (int j = -4; j <= 4; j++){
			total += tile1[tilePos + i * segLengthX[0] + j];
