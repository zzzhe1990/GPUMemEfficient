...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
	  ...
	  cudaCheckReturn(cudaMalloc((void **) &dev_arr1, (len1 + 2) * (4100) * sizeof(int)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_arr2, (len1 + 2) * (4100) * sizeof(int)));
	  
	  if (padd <= 4101) {
	    cudaCheckReturn(cudaMemcpy(dev_arr1, arr1, (len1 + 2) * (4100) * sizeof(int), cudaMemcpyHostToDevice));
	    cudaCheckReturn(cudaMemcpy(dev_arr2, arr2, (len1 + 2) * (4100) * sizeof(int), cudaMemcpyHostToDevice));
	  }
	...
	printf("execution time: %lf\n", t3);
	  ...
	  if (padd <= 4101) {
	    cudaCheckReturn(cudaMemcpy(arr1, dev_arr1, (len1 + 2) * (4100) * sizeof(int), cudaMemcpyDeviceToHost));
	    cudaCheckReturn(cudaMemcpy(arr2, dev_arr2, (len1 + 2) * (4100) * sizeof(int), cudaMemcpyDeviceToHost));
	  }
