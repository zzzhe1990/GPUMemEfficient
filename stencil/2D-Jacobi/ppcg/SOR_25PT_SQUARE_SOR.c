	...
	for (int t=0; t < trial; t += 2){
		for (int r = padd; r < len1; r++){
			for (int c = padd; c < len2; c++){
				arr2[r][c] += (
	arr1[r-2][c-2] + arr1[r-2][c-1] + arr1[r-2][c] + arr1[r-2][c+1] + arr1[r-2][c+2]
      + arr1[r-1][c-2] + arr1[r-1][c-1] + arr1[r-1][c] + arr1[r-1][c+1] + arr1[r-1][c+2]
      +	arr1[r][c-2] + arr1[r][c-1] + arr1[r][c] + arr1[r][c+1] + arr1[r][c+2]
      +	arr1[r+1][c-2] + arr1[r+1][c-1] + arr1[r+1][c] + arr1[r+1][c+1] + arr1[r+1][c+2]
      +	arr1[r+2][c-2] + arr1[r+2][c-1] + arr1[r+2][c] + arr1[r+2][c+1] + arr1[r+2][c+2]
					    ) / 25;
			}	
		...
		for (int r = padd; r < len1; r++){
			for (int c = padd; c < len2; c++){
				arr1[r][c] += (
	arr2[r-2][c-2] + arr2[r-2][c-1] + arr2[r-2][c] + arr2[r-2][c+1] + arr2[r-2][c+2]
      + arr2[r-1][c-2] + arr2[r-1][c-1] + arr2[r-1][c] + arr2[r-1][c+1] + arr2[r-1][c+2]
      +	arr2[r][c-2] + arr2[r][c-1] + arr2[r][c] + arr2[r][c+1] + arr2[r][c+2]
      +	arr2[r+1][c-2] + arr2[r+1][c-1] + arr2[r+1][c] + arr2[r+1][c+1] + arr2[r+1][c+2]
      +	arr2[r+2][c-2] + arr2[r+2][c-1] + arr2[r+2][c] + arr2[r+2][c+1] + arr2[r+2][c+2]
					    ) / 25;
			}	
