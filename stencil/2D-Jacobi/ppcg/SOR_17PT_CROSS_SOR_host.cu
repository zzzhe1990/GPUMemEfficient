...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
	  ...
	  cudaCheckReturn(cudaMalloc((void **) &dev_arr1, (padd >= 4112 ? len1 : len1 + 4) * (4112) * sizeof(int)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_arr2, (padd >= 4112 ? len1 : len1 + 4) * (4112) * sizeof(int)));
	  
	  if (padd <= 4114) {
	    cudaCheckReturn(cudaMemcpy(dev_arr1, arr1, (padd >= 4112 ? len1 : len1 + 4) * (4112) * sizeof(int), cudaMemcpyHostToDevice));
	    cudaCheckReturn(cudaMemcpy(dev_arr2, arr2, (padd >= 4112 ? len1 : len1 + 4) * (4112) * sizeof(int), cudaMemcpyHostToDevice));
	  }
	...
	printf("execution time: %lf\n", t3);
	  if (padd <= 4114) {
	    cudaCheckReturn(cudaMemcpy(arr1, dev_arr1, (padd >= 4112 ? len1 : len1 + 4) * (4112) * sizeof(int), cudaMemcpyDeviceToHost));
	    cudaCheckReturn(cudaMemcpy(arr2, dev_arr2, (padd >= 4112 ? len1 : len1 + 4) * (4112) * sizeof(int), cudaMemcpyDeviceToHost));
	  }
