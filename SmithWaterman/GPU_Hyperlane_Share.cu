		...
		for (int i=0; i<highY; i++){
			...
			if(thread <= concurrency){
				...
				table[idx] = max(table[idx-1]-2, 
					max(table[idx-segLengthX]-2, 
					max(table[idx-segLengthX-1]+s(dev_arr1[idxx], dev_arr2[idxy]), 0)));
			}
			__syncthreads();
		}
...
//New feature warp level syncronize or thread group syncronize may solve the problem.
			if (thread < tileY){
				...
				idxy = arrY + thread;
				table[idx] = max(table[idx-1]-2, 
					max(table[idx-segLengthX-1]-2, 
					max(table[idx-segLengthX-2]+s(dev_arr1[idxx], dev_arr2[idxy]), 0)));
			}
			lvlStartAddress += 1;
			__syncthreads();
		}
...
int SW(int n1, int n2, int *arr1, int *arr2, int paddX, int paddY, int *table, int tX, int tY){
	cudaSetDevice(0);
	...
	cout << "GPU: "	 << gpuinfo.name << ", multiprocessor count: " << gpuinfo.multiProcessorCount<< endl;
