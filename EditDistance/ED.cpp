...
int ED(int n1, int n2, int *arr1, int *arr2, int *table){
	int last;
	...
	for (int y=0; y < n1; y++){
		for (int x=0; x < n2; x++){
			...
			int yy = y+1;
			int idx = yy * (n2+1) + xx;
			table[idx] = min(table[idx-n2-1]+1, min(table[idx-1]+1, table[idx-n2-2]+1));
			if (arr1[x] == arr2[y])
				table[idx] = table[idx-n2-2];
		}
	}
/*
	//display table
